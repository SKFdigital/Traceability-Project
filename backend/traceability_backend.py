    ...
    try:
        ...
        for _, df in mo_sheets.items():
            ...
            for _, row in df.iterrows():
                ...
                raw_mo_data.append({
                    "mo_group": mo_group, "full_mo": raw_mo, "variant": final_variant, "qty_req": qty_req
                })

                # CRITICAL FIX: Group ONLY by MO Group to guarantee all data maps properly
                if mo_group not in summary_aggregation:
                    base_prod_display = final_variant if final_variant else "Generic Product"
                    summary_aggregation[mo_group] = {
                        "mo": mo_group, "base_product": base_prod_display, "qty_req": 0.0,
                        "sho_qty": 0.0, "sho_date": None, "tb_qty": 0.0, "tb_date": None,
                        "ch_qty": 0.0, "ch_date": None
                    }
                summary_aggregation[mo_group]["qty_req"] += qty_req
        ...
        for _, df in jobwork_sheets.items():
            ...
            for _, row in df.iterrows():
                ...
                if mo_group in summary_aggregation:
                    summary_aggregation[mo_group]["sho_qty"] += sho_qty
                    summary_aggregation[mo_group]["tb_qty"] += tb_qty
                    if sho_date: summary_aggregation[mo_group]["sho_date"] = sho_date
                    if tb_date: summary_aggregation[mo_group]["tb_date"] = tb_date
        ...
        for _, df in all_channels.items():
            ...
            for _, row in df.iterrows():
                ...
                raw_ch_data.append({
                    "mo_group": mo_group, "full_mo": raw_mo, "variant": variant, 
                    "ch_qty": ch_qty, "ch_date": ch_date
                })

                if mo_group in summary_aggregation:
                    if ch_qty > summary_aggregation[mo_group]["ch_qty"]:
                        summary_aggregation[mo_group]["ch_qty"] = ch_qty
                    if ch_date: summary_aggregation[mo_group]["ch_date"] = ch_date

        compiled_summary = []
        for _, data in summary_aggregation.items():
            status = "Completed" if (data["ch_qty"] >= data["qty_req"] and data["qty_req"] > 0) else ("In Process" if data["sho_qty"] > 0 else "Yet to Start")
        ...
        compiled_summary.sort(key=lambda x: x["mo"])
...
def get_traceability_flow(mo: str):
    ...
    # 1. Map all targets directly from MO data
    for r in GLOBAL_RAW_RECORDS["mo_data"]:
        if r["mo_group"] == search_group:
            v = r["variant"] if r["variant"] else "Generic Variant"
    ...
    # Helper function to prevent mismatched variant rows in drilldown
    def find_best_variant_match(target_v, current_map):
        if target_v in current_map: return target_v
        for k in current_map.keys():
            if target_v and target_v in k: return k
            if target_v and k in target_v: return k
        if len(current_map) == 1:
            return list(current_map.keys())[0]
        return target_v

    # 2. Map JobWork Quantities safely
    for r in GLOBAL_RAW_RECORDS["jw_data"]:
        if r["mo_group"] == search_group:
            raw_v = r["variant"] if r["variant"] else "Generic Variant"
            v = find_best_variant_match(raw_v, variant_map)
    ...
    # 3. Map Channel Quantities safely
    ...
    for k, v in variant_map.items():
        ...
        status = "Completed" if (v["ch_qty"] >= v["qty_req"] and v["qty_req"] > 0) else ("In Process" if v["sho_qty"] > 0 else "Yet to Start")
